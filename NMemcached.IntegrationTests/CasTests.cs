...
namespace NMemcached.IntegrationTests
{
	...
	public class CasTests : AbstractMemcachedIntegrationTest
	{
		...
		public void Can_ask_server_to_modify_using_cas_value()
		{
			...
			long unique;
			client.Gets("ayende", out unique);
			CacheOperationResult set = client.CheckAndSet("ayende", "blah", unique);
			Assert.AreEqual(CacheOperationResult.Stored, set);
		...
		public void Will_not_update_value_if_does_not_match_cas_value()
		{
			...
			long unique;
			client.Gets("ayende", out unique);
			CacheOperationResult set = client.CheckAndSet("ayende", "blah", unique + 2);
			Assert.AreEqual(CacheOperationResult.Exists, set);
		...
		{
			CacheOperationResult set = client.CheckAndSet("ayende", "blah", 12L);
			Assert.AreEqual(CacheOperationResult.NotFound, set);
		...
		public void When_trying_to_cas_a_deleted_value_will_return_not_stored()
		{
			...
			CacheOperationResult set = client.CheckAndSet("ayende", "blah", unique);
			Assert.AreEqual(CacheOperationResult.NotStored, set);
			Assert.IsNull(client.Get("ayende"));
		}
	}
}
